...
namespace ArtworkSharing.DAL.Extensions;

public static class DbContextExtension
{
    private static readonly object ConnectionLocked = new();
    ...
    /// <param name="prependDefaultSchema">
    ///     Prepend the default schema name to <paramref name="storedProcName" /> if explicitly
    ///     defined in <paramref name="context" />
    /// </param>
    /// <returns></returns>
        ...
        bool prependDefaultSchema = true)
    {
        ...
        if (prependDefaultSchema)
        {
            ...
            if (schemaName != null) storedProcName = $"{schemaName}.{storedProcName}";
        }
    ...
    public static DbCommand WithSqlParam(this DbCommand cmd, string paramName, object paramValue,
        ParameterDirection direction = ParameterDirection.Input, Action<DbParameter> configureParam = null)
    {
    ...
    {
        if (handleResults == null) throw new ArgumentNullException(nameof(handleResults));
        ...
        using (command)
        {
            ...
            {
                if (manageConnection) command.Connection.Close();
            }
        }
    }
    ...
    public static async Task ExecuteStoredProcAsync(this DbCommand command, Action<SprocResults> handleResults,
        CommandBehavior commandBehaviour = CommandBehavior.Default, CancellationToken ct = default,
    ...
    public static async Task<List<T>> ExecuteForListAsync<T>(this DbCommand command,
        CommandBehavior commandBehaviour = CommandBehavior.Default, CancellationToken ct = default,
        bool manageConnection = false)
        where T : class
    {
        ...
        await command.ExecuteStoredProcAsync(d => { result = d.ReadNextListOrEmpty<T>(); }, commandBehaviour, ct,
            manageConnection);

        return result;
    }

    public static async Task<T> ExecuteForFirstAsync<T>(this DbCommand command,
        CommandBehavior commandBehaviour = CommandBehavior.Default, CancellationToken ct = default,
        bool manageConnection = false)
        where T : class
    {
        ...
        await command.ExecuteStoredProcAsync(d => { result = d.ReadNextListOrEmpty<T>().FirstOrDefault(); },
            commandBehaviour, ct, manageConnection);
                    ...
                    foreach (var prop in props)
                        if (colMapping.ContainsKey(prop.Name.ToLower()))
                        {
                            ...
                            if (column?.ColumnOrdinal != null)
                            {
                                ...
                                if (prop.CanWrite) prop.SetValue(obj, val == DBNull.Value ? null : val);
                            }
                        }
