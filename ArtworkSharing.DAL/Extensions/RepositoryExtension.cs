...
public static class RepositoryExtension
{
    ...
    ///     Get first record for the tenant
    /// </summary>
    public static async Task<T> SingleOrDefaultAsync<T>(this IRepository<T> repository,
        Expression<Func<T, bool>> predicate)
        where T : class
    {
    ...
    /// </summary>
    public static async Task<T> LastOrDefaultAsync<T>(this IRepository<T> repository,
        Expression<Func<T, bool>> predicate)
        where T : class
    {
    ...
    /// </summary>
    public static IIncludableQueryable<T, TProperty> Include<T, TProperty>(this IRepository<T> repository,
    ...
    public static void Load<T, TProperty>(this IRepository<T> repository, T entity,
    ...
    private static void LoadEntity<T, T1, TProperty>(this IRepository<T> repository, T1 entity,
        Expression<Func<T1, TProperty>> propertyExpression)
        where T : class where TProperty : class where T1 : class
    {
        repository.DbContext.Entry(entity).Reference(propertyExpression).Load();
    }

    public static void LoadEntities<T, T1, TProperty>(this IRepository<T> repository, IEnumerable<T1> entities,
        Expression<Func<T1, TProperty>> propertyExpression)
        where T : class where TProperty : class where T1 : class
    {
        foreach (var entity in entities) repository.LoadEntity(entity, propertyExpression);
    }
}