...
public class ManageArtWorkController : ControllerBase
{
    ...
    public ManageArtWorkController(IArtistService artistService, IArtworkService artworkService,
        ILogger<ManageOrderArtistController> logger)
    {
    ...
    public async Task<ActionResult<ManageOrderArtistController>> GetCombinedEntityById(Guid entityId, int page)
    {
        try
        {
            ...
            if (artists == null) return NotFound("Artist not found");
    ...
    public async Task<IActionResult> Update([FromBody] Artwork artworkInput)
    {
        try
        {
            ...
            if (existArtwork == null) return NotFound("Artwork not found");
