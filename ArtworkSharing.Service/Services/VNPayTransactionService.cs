...
namespace ArtworkSharing.Service.Services
{
    public class VNPayTransactionService : IVNPayTransactionService
    {
        ...
        public VNPayTransactionService(IConfiguration configuration, IHttpContextAccessor httpContext, IUnitOfWork unitOfWork)
        {
            _uow = unitOfWork;
        ...
        private string GetFromQuery(string query)
        {
            ...
            foreach (KeyValuePair<string, string> item in pParams)
            {
                if (item.Key.IsNullOrEmpty() || item.Value.IsNullOrEmpty() || item.Key == "vnp_SecureHashType" || item.Key == "vnp_SecureHash") continue;
        ...
        public async Task<List<VNPayTransactionViewModel>> GetVNPayTransactions(VNPayFilter vNPayFilter)
        {
            ...
            if (vNPayFilter.PayDateFrom != null!)
            {
                vtrans = vtrans.Where(x => x.PayDate >= vNPayFilter.PayDateFrom);
            }
            if (vNPayFilter.PayDateTo != null!)
            {
                vtrans = vtrans.Where(x => x.PayDate <= vNPayFilter.PayDateTo);
            }
        ...
        {
            var transVNPay = await _uow.VNPayTransactionRepository.Include(x => x.Transaction).FirstOrDefaultAsync(x => x.Id == id);
            if (transVNPay == null)
            {
                return new VNPayResponseModel
                {
            ...
            VNPayRefundRequestModel vNPayRefund = new VNPayRefundRequestModel
            {
            ...
            VNPayRefundResponseModel vNPayRefundResponse = JsonConvert.DeserializeObject<VNPayRefundResponseModel>(strData)!;

            if (vNPayRefundResponse.vnp_ResponseCode == "00")
            {
                VNPayTransactionRefund vNPayTransactionRefund = new VNPayTransactionRefund
                {
                    Id = transVNPay.Id,
                    Amount = vNPayRefundResponse.vnp_Amount,
                    BankCode = vNPayRefundResponse.vnp_BankCode,
                    PayDate = DateTime.ParseExact(vNPayRefundResponse.vnp_PayDate, "yyyyMMddHHmmss", CultureInfo.InvariantCulture),
        ...
        public async Task<VNPayTransactionViewModel> GetVNPayTransactionByTransactionId(Guid id)
            => AutoMapperConfiguration.Mapper.Map<VNPayTransactionViewModel>(await _uow.VNPayTransactionRepository.FirstOrDefaultAsync(x => x.TransactionId == id));
    }
}
