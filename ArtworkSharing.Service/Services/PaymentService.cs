...
public class PaymentService : IPaymentService
{
    ...
    private string GetFromQuery(string query)
    {
        ...
        var pParams = new SortedList<string, string>();
        ...
        {
            if (item.Key.IsNullOrEmpty() || item.Value.IsNullOrEmpty() || item.Key.StartsWith("vnp_") ||
                item.Key == "vnp_SecureHashType" || item.Key == "vnp_SecureHash") continue;
