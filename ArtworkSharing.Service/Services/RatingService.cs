...
public class RatingService : IRatingService
{
    ...
    public async Task<IList<RatingViewModel>> GetAll()
    {
        ...
        return AutoMapperConfiguration.Mapper.Map<IList<RatingViewModel>>(
            await queryableData.ToListAsync(CancellationToken.None));
    }

    public async Task<RatingViewModel> GetOne(Guid RatingId)
    {
        return AutoMapperConfiguration.Mapper.Map<RatingViewModel>(
            await _unitOfWork.PackageRepository.FirstOrDefaultAsync(x => x.Id == RatingId));
    }
